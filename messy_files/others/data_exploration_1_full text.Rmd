---
title: "Amazon in Presidential Speeches in Brazil: data exploration for full texts"
...
f_speaker <- kableExtra::kbl(f_speaker, caption = "Top 10 Words Per President") %>%
  kableExtra::kable_classic(full_width = F, html_font = "Times New Roman")
f_speaker # not super informative ...
```
...
topten_topic <- aggregate(amazon_lda10$term, list(amazon_lda10$topic), paste, collapse=", ") %>%
  ...
  kableExtra::kable_classic(full_width = F, html_font = "Times New Roman")
topten_topic # "ambiente" (environment) appears only in topic 7 ...
# Agriculture, on the other hand, is the first cluster...
...
# Lula more positive about in speeches surrounding the Amazon than others...
...
# Plot NRC
ggplot(amazon_sent_nrc, aes(x = reorder(id, as.numeric(year)), y = value, fill = sentiment)) +
...
# Lula 2010 appears to have spoken more and used more "carried" words related to certain sentiment according to dictionary in texts mentioning the Amazon.
```
...
Amazon_lc <- mutate(Amazon_speeches, stage = case_when(grepl("Amazonas|Para|Roraima|Acre|Amapa|Rondonia|Mato-Grosso|Tocantins", location) ~ "local", grepl("Alagoas|Bahia|Ceará|Distrito Federal|Goias|Maranhao|Minas Gerais|Paraiba|Parana|Pernambuco|Piaui|Rio de Janeiro|Rio Grande do Norte|Rio Grande do Sul|Santa Catarina|Sao Paulo|Brazil|Sergipe", location) ~ "national", grepl("Argentina|Chile|China|Colombia|Cuba|European|France|Hamburg|India|Israel|Jamaica|Nicaragua|Oman|ONU|Panama|Peru|Portugal|Senegal|Suriname", location) ~ "international"))
Amazon_lc$stage <- stringr::str_replace_na(Amazon_lc$stage, "Non Identified")
summary(as.factor(Amazon_lc$stage)) # A lot of international observations ...
...
amazon_lda_lc <- topicmodels::LDA(amazon_dtm_lc, k = 20, control = list(seed = 1234)) # set k = 20
...
amazon_lda_lcs <- tidy(amazon_lda_lc, matrix = "gamma") %>%
  arrange(desc(gamma)) %>%
  group_by(document) %>%
  filter(row_number() %in% 1:8) %>% # get the top 3 topics per location
  arrange(document) %>% 
  select(-gamma) %>% 
  distinct()
...
amazon_lda_topic <- dplyr::left_join(topten_lc, amazon_lda_topic, by = "Topic") %>%
  ...
  kableExtra::kable_classic(full_width = F, html_font = "Times New Roman")
amazon_lda_topic # Do you see anything interesting?
```

This is where I stop since, overall, this appears not to be very helpful ...
But how about we try the a similar script with only the matches themselves instead of full texts?
