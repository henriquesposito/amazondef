...
ama_mx %>%
  count(mx_cat, mx_cat2,location_cat, year) %>%
  group_by(year) %>%
  mutate(perc = n / sum(n)) %>% 
  filter(location_cat=="Amazonian States"|location_cat=="International")%>%
  filter(mx_cat2 == "Pure-types") %>%
  ggplot() +
  ...
  labs(x = "", y = "", title ="Relationship between problem-constructions and deforestation rates",
       ...
       caption = "Curves in the plot were smoothed using loess method.")+
  theme_am()+
  geom_col(amazon_def_year, mapping = aes(x = year, y = def_year/80,fill=extremes),alpha=.5)+
  scale_fill_manual(values=c("lightgrey", "darkolivegreen"))+
  annotate("text", x = 2008, y = 0.35, label = "27'772 km2\ndeforested" , size = 3, color="darkolivegreen", face="bold")+
  annotate("text", x = 2016, y = 0.05, label = "4'571 km2\ndeforested", size = 3, color="darkolivegreen", face="bold")
```
