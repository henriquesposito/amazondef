...
pure_types_time <- ama_mx %>%
  ...
  ggplot2::annotate(geom = "rect", xmin = hp$beg, xmax = hp$end, ymin = 0, ymax = 0.40,
           ...
           alpha = 0.1) +
  ggplot2::annotate(geom = "text", x = hp$label, y = 0.01, vjus = 0.3, hjust = 0.3, angle = 25,
...
ama_model <- filter(ama_model, location_cat != "Non Identified", mx_cat != "Other Mixed-types") %>%
  mutate(mx_cat = factor(mx_cat, levels = c("Pure Conservation", "Pure Economic Integration",
         ...
         con_vs_all = ifelse(mx_cat == "Pure Conservation", 1, 0),
...
# plot
mplot <- ama_model %>%
  select(con_vs_all, km_to_manaus) %>%
  ggplot(aes(x = km_to_manaus, y = con_vs_all)) +
  geom_smooth(se = FALSE, size = .5, color = "darkgreen") +
  scale_y_continuous(labels = percent_format()) +
  geom_vline(xintercept = c(1933.78, 5075.48, 8306.76, 9678.78, 15904.47), linetype = "dotted") +
  ggplot2::annotate(geom = "text", x = c(1933.78, 5075.48, 8306.76, 9678.78, 15904.47), y = 0.5,
                    label = c("Brasilia", "New York", "Paris", "Cape Town", "Beijing"), alpha = 0.4) +
  labs(x = "Distance to Manaus (km)",
       y = "Environmental conservation",
       title = "Amazon constructions as an issue of pure environmental conservation\nin relation to distance to Manaus",
       subtitle = "Distance as a straight line from state, or country, capital to Manaus in kilometers",
...
mplot
mplot_all <- ama_model %>%
  select(con_vs_all, EI_vs_all, SD_vs_all, sov_vs_all, km_to_manaus) %>%
  rename("Pure Conservation" = con_vs_all,
         "Pure Economic Integration" = EI_vs_all,
         "Pure Social Development" = SD_vs_all, 
         "Pure Sovereignty" = sov_vs_all) %>% 
  pivot_longer(-km_to_manaus, names_to = "variable", values_to = "value") %>% 
  ggplot(aes(x = km_to_manaus, y = value, color = variable)) +
  ...
  ggplot2::annotate(geom = "text", x = c(1933.78, 5075.48, 8306.76, 9678.78, 15904.47), y = 0.5,
                    label = c("Brasilia", "New York", "Paris", "Cape Town", "Beijing"), alpha = 0.4) +
  labs(x = "Distance to Manaus (km)",
       y = "Problem Construction",
       title = "Amazon problem constructions in relation to distance to Manaus",
       subtitle = "Distance as a straight line from state, or country, capital to Manaus in kilometers",
       caption = "Curve in the plot is smoothed using loess method.") +
    theme(panel.background = element_rect("white", "black", .5, "solid"),
          panel.grid.major = element_line(color = "grey", size = 0.3, linetype = "solid"),
...
mplot_all
# model
model <- glm(con_vs_all ~ km_to_manaus + election_year + def_year + AAI,
