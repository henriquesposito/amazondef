...
model_logit <- glm(con_vs_all ~ km_to_manaus + election_year + def_year + AAI,family=binomial(link = "logit"), data = ama_model)

model_lm <- lm(con_vs_all ~ km_to_manaus + election_year + def_year + AAI, data = ama_model)

# summary statistics
stargazer::stargazer(model_lm, type = "text")
...
ama_model$predicted = predict(model_lm, ama_model, type = "response")
ggplot(ama_model, aes(x = km_to_manaus, y = predicted)) + 
  geom_jitter(alpha=.5)+
  geom_smooth(size = .5, color = "darkgreen") +
    labs(x = "Distance from the Amazon (km)",
       y = "Share of environmental conservation",
       title = "Amazon constructions as an issue of  environmental conservation\nin relation to distance from the Amazon",
       subtitle = "Distance as a straight line from state, or country, capital from the Amazon in kilometers",
