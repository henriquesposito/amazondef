...
# Load data
##### We are missing many observations for Collor from 1991 and 1992...
##### These are not in the repository for some reason...
##### Should we try and complete or should we at least disclose these in a footnote?
...
We need to protect the Amazon from foreign interests. We need to exploit the Amazon’s natural resources. We need to provide better living standards for the people in the Amazon. We need to preserve the Amazon as a standing ecosystem. Historically, the Brazilian government approached the Amazon region, forest and its people as a policy object.Different governmental programs contained an assumptions of what had to be solved in the Amazon, or in other words, it represents the Amazon as a particular problem. These specific problems touch on shared imaginaries of the Amazon as part of the larger socio-cultural history of the country as, for example, in relation to issues of national sovereignty, economic integration, social development, or environmental conservation.
...
Additionally, @bacchi2009[p. 10] argues that policies have a cultural dimension as “it takes shape within specific historical and national or international contexts”. The existence or proposal of a policy implies that there is a (public) problem that needs (governmental) action to be fixed. The alleged problem is not always explicitly stated in policy: policies are represented as solutions for implicit problems.@hirschman1975[p. 388] exemplifies chosen problems with the case of the construction of Brasilia. Building Brasilia could solve a problem of regional inequality, a problem of a dormant economy without state investment, a problem of political representation, or all three. It is up to the discretion of the government to highlight or not the implicit problem a policy solves. A same policy object (e.g. Brasilia, the Amazon, criminality, poverty), thus, can be constructed as pressing or not depending on how a government represents it.
...
Closely related to economic integration, social development is a problem-construction that is often neglected while describing policy cycles or phases. While this might be related to social policies being more geared towards the whole nation, there are instances of specific social policies for the Amazon.Governments can emphasize the lack of hospitals, sanitation, and schools concerning peoples’ dignity, standards of living, and other constitutional rights when speaking about the Amazon. This is not only relevant as a construction, but speaks, minimally, to democratic concerns over providing electorates, especially in the Amazonian region, with better standards of living than previously.
...
# Word frequency year
popc <- filter(pop, amazon_speech == 1) %>%
  select(year, text) %>% 
  group_by(year) %>% 
  rename(doc_id = year)
popc$text <- stringr::str_replace_all(popc$text, "meio ambiente", "meioambiente")
popc$text <- stringr::str_replace_all(popc$text, "forcas armadas", "forcasarmadas")
popc$text <- stringr::str_replace_all(popc$text, "estados unidos", "estadosunidos")
popc$text <- stringr::str_replace_all(popc$text, "rio de janeiro", "riodejaneiro")
popc$text <- stringr::str_replace_all(popc$text, "direitos humanos", "direitoshumanos")
popc$text <- stringr::str_replace_all(popc$text, "rio grande do sul", "riograndedosul")
popc$text <- stringr::str_replace_all(popc$text, "copa do mundo", "copadomundo")
popc$text <- stringr::str_replace_all(popc$text, "sao paulo", "saopaulo")
popc$text <- stringr::str_replace_all(popc$text, "rio grande do norte", "riograndedonorte")
popc$text <- stringr::str_replace_all(popc$text, "floresta amazonica", "florestaamazonica")
popc$text <- stringr::str_replace_all(popc$text, "regiao amazonica", "regiaoamazonica")
popc$text <- stringr::str_replace_all(popc$text, "mato grosso|mato-grosso", "matogrosso")
popc$text <- stringr::str_replace_all(popc$text, "minas gerais", "minasgerais")
popc$text <- stringr::str_remove_all(popc$text, " [a-z] ")
popc$text <- tm::removeWords(tm::removePunctuation(popc$text),
                             c(tm::stopwords("pt"), 
                               "nao", "amazonas", "voce", "vao", "dar", "du",
                               "our", "amazonia", "governo", "amazonica", "ja", "aqui", "tambem",
                               "no", "hoje", "presidente", "governador", 
                               ...
                               "todos", "companheiro", "senhores", "alguns",
                               "companheiras", "companheiros", "companheira", "entao",
                               "senhor", "senhora", "agora", "senhores", "senhoras", "caro",
                               "grosso","ministro", "ministra", "josé", "jose",
                               "palacio", "planalto", "pronunciamento", "sobre",
                               "ministros", "luiz", "disse", "fizemos", "nesse",
                               "antonio", "vou", "eduardo braga", "pais", "coisa",
                               "então", "pouco", "pode", "sera", "dias",
                               "outro", "onde", "discurso", "parte", "fala",
                               "anos", "cada", "todo", "visite site", "disse",
                               "site secretaria", "minas gerais", "lula silva",
                               "inácio lula", "amapa", "bahia", "é", "lá", "et", "in",
                               "httpwwwimprensaplanaltogovbr", "secretaria", "visitesite",
                               "ta", "tudo", "pessoas", "falar", "neves", "tancredo",
                               "brasiliadf", "cardoso", "httpwwwinfoplanaltogovbr", 
                               "sip", "and", "to", "en", "des", "el", "le", "les", "dia",
                               "palavra dorsdne", "fernando henrique", "dorsdne eete",
                               "inacio lula", "lula silva", "inacio", "republica",
                               "quegente", "muitas vezes", "ainda", "realmente",
                               "assim", "possa", "coisas", "apenas", "acho", "ano",
                               "vez", "portanto", "todas", "questao", "lado", "outros",
                               "prezado", "vem", "sempre", "sarney", "luis", "serao", 
                               "tao", "meses", "quase", "diante", "brasil", "sendo",
                               "rsdneennoerqe", "brasil", "neste", "digo", "quase", 
                               "queremos", "nesse", "nessa", "mim", "eete", "obrigado", "muita"))
popc <- tm::VCorpus(tm::DataframeSource(popc))
popn <- tidytext::tidy(popc) %>%
  unnest_tokens(word, text) %>% 
  group_by(id, word) %>% 
  summarise(count = n()) %>% 
  arrange(-count) %>% 
  slice(1:10)
popn <- aggregate(popn$word, list(popn$id), paste, collapse=", ") %>% 
  rename(Year = "Group.1", Terms = "x")
tibble(popn)
##### LDA per year just in case
pop_dtm  <- DocumentTermMatrix(popc)
pop_lda <- topicmodels::LDA(pop_dtm, k = 10, control = list(seed = 1234))
pop_lday <- tidy(pop_lda, matrix = "gamma") %>%
  arrange(desc(gamma)) %>%
  group_by(document) %>%
  filter(row_number() %in% 1:1) %>% # get the top 3 topics per speaker
  arrange(document) %>%
  mutate(gamma = round(gamma, 1)) %>%
  distinct() %>% 
  rename (Year = document)
pop_10 <- tidy(pop_lda, matrix = "beta") %>%
  arrange(desc(beta)) %>%
  group_by(topic) %>%
  filter(row_number() %in% 1:10) %>%
  arrange(topic)
pop_10 <- aggregate(pop_10$term, list(pop_10$topic), paste, collapse=", ") %>%
  rename(topic = "Group.1", Terms = "x")
popt <- dplyr::left_join(pop_10, pop_lday, by = "topic") %>%
  select(Year, Terms) %>% 
  arrange(Year)
tibble(popt)
```
  ...
  mutate(mx_cat = factor(mx_cat, levels = c("Pure Conservation", "Pure Economic Integration",
         ...
         location_cat = factor(ifelse(grepl("Bolivia|Peru|Ecuador|Colombia|Venezuela|Guyana|Suriname",
                               ...
                               levels = c("International", "Amazonian States",
                                          "Amazonian Countries", "Brasilia", "Non Amazonian States")))
# model
