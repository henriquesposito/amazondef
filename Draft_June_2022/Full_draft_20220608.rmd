...
# Same plot with location
perc_in <- pop %>%
  mutate(location_cat = ifelse(grepl("International|Non Identified", location_cat),
                               "Outside of Brazil", "Within Brazil")) %>%
  count(amazon_speech, year, location_cat) %>%
  group_by(year) %>%
  mutate(perc = n / sum(n)) %>%
  filter(amazon_speech == 1) %>%
  select(c(perc, amazon_speech, year, location_cat))
Amazon_year_in <- perc_in %>% ggplot(., aes(x = year, y = perc, fill = location_cat)) +
  geom_line(aes(color = location_cat)) +
  scale_y_continuous(labels = percent_format()) +
  geom_text(aes(x = 1988.7, y = 0.34, label = "Chico Mendes"), size = 3) +
  geom_text(aes(x = 1992, y = 0.27, label = "Earth Summit"), size = 3) +
  geom_text(aes(x = 2009, y = 0.24, label = "Copenhagen Summit"), size = 3) +
...
model_logit_sov <- glm(sov_vs_all ~ km_to_manaus + election_year + def_year + AAI,
                   family=binomial(link = "logit"), data = ama_model)
# summary statistics, in appendix for now
#stargazer::stargazer(model_logit_con, model_logit_ei, model_logit_sd, model_logit_sov,
#                     type = "text") # Negative for EI and SD
...
# Plot model
ama_model$predicted_con = predict(model_logit_con, ama_model, type = "response")
ama_model$predicted_ei = predict(model_logit_ei, ama_model, type = "response")
ama_model$predicted_sd = predict(model_logit_sd, ama_model, type = "response")
ama_model$predicted_sov = predict(model_logit_sov, ama_model, type = "response")
# Plot all regressions, just in case
...
#           legend.title = element_blank(),
#           plot.subtitle = element_text(color = "black", size = 9, face = "plain"))
# # Environmental conservation and location
ggplot(ama_model, aes(x = km_to_manaus, y = predicted_con)) +
  geom_jitter(alpha=.5)+
  geom_smooth(size = .5, color = "darkgreen") +
  geom_vline(xintercept = c(1933.78, 5075.48, 8306.76), linetype = "dotted") +
  ggplot2::annotate(geom = "text", x = c(1933.78, 5075.48, 8306.76), y = 0.5,
                    label = c("Brasilia", "New York", "Paris"), alpha = 0.4) +
  labs(x = "Distance from the Amazon (km)",
       y = "The Amazon as an issue of environmental conservation",
       title = "Share of speeches that constructed the as an issue of pure environmental conservation\nin relation to distance from Manaus",
       subtitle = "Distance as a straight line from state or country's capital to Manaus in kilometers",
       caption = "Curve in the plot is smoothed using loess method.") +
    theme(panel.background = element_rect("white", "black", .5, "solid"),
          panel.grid.major = element_line(color = "grey", size = 0.3, linetype = "solid"),
...
kbl(t(model_comp), booktabs = T,caption = "Logistic Regression Models Compared", align = "c") %>%
  kable_styling(latex_options = c("striped", "scale_down"),
                ...
                footnote(general = c("The regression coefficients are displayed in log odds and standard errors are displayed in parentheses.",
