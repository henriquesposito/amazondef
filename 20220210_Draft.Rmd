...
# Still some overlap...Also, there were 976 context units not matched at all...
# Let's see some of which is not matched
o <- dplyr::filter(AS_long, other == 1) %>% dplyr::select(president, year, context)
dplyr::slice_sample(o, n = 20)
o$context[1:20] # Number 12 and 17 (riquezas no subsolo/mineral reserves) development?
o$context[500:520] # Number 7, development or preservation or neither? 10 (chico mendes) preservation? 14 sovereingty? 15 development and sovereingty? 17 and 18 development? 19 and 20 conservation? 21 development?
o$context[940:960] # 1 (gas and niobio) development? 3 sovereignty? 6 development and preservation? 7 conservation? A lot of Bolsonaro's mentions about the Amazonia not burning, should we code these or not? 
```
...
```{r}
# STM model, just for the sake of it, to see if we can use some of the metadata better to find topics and correlations ...
library(stm)
data <- AS_long %>%
  select(title, president, year, location, context, sovereignty, development, conservation, other) %>% 
  tidyr::pivot_longer(sovereignty:other) %>% 
  dplyr::rename(documents = title, text = context, narrative = name) %>%
  arrange(documents, text)
data$text <- tm::removeWords(am$text, c(tm::stopwords("pt"), "nao", "amazonas",
                                          "amazonia", "governo", "amazonica", "ja", "aqui", "tambem",
                                          "no", "hoje", "presidente", "governador", "regiao", 
                                          "sao", "voces", "ser", "bem", "la", "so", "desta", "porque",
                                          "fazer", "quero", "ter", "vamos", "the", "of", "ha", "ai",
...
vocab <- out$vocab
meta <- out$meta
plotRemoved(processed$documents, lower.thresh = seq(1, 100, by = 100))
out <- prepDocuments(processed$documents, processed$vocab, processed$meta, lower.thresh = 10)
poliblogPrevFit <- stm(documents = out$documents, vocab = out$vocab, K = 5, # five clusters
                       prevalence = ~narrative + president, # model for narrative and president
                       max.em.its = 75, data = out$meta, init.type = "Spectral")
poliblogSelect <- selectModel(out$documents, out$vocab, K = 5, ~narrative + president,
                              max.em.its = 75, data = out$meta, runs = 20, seed = 8458159)
plotModels(poliblogSelect, pch = c(1, 2, 3, 4), legend.position = "bottomright")
labelTopics(poliblogPrevFit, c(1, 2, 3, 4, 5)) # get labels for each model
out$meta$narrative <- as.factor(out$meta$narrative)
prep <- estimateEffect(1:5 ~narrative + president, poliblogPrevFit, meta = out$meta, uncertainty = "Global")
summary(prep, topics = c(1:5)) # Ok, interesting
