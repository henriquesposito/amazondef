...
library(dplyr)
final_alebeled_data <- readRDS("~/Documents/GitHub/amazondef/data/final_alebeled_data.Rds")
final_alebeled_data$false_positives <- as.factor(ifelse(final_alebeled_data$false_positives > 0.5, 1, 0))
final_alebeled_data$sov <- as.factor(ifelse(as.numeric(final_alebeled_data$sov) > 0.5, 1, 0))
final_alebeled_data$EI <- as.factor(ifelse(as.numeric(final_alebeled_data$EI) > 0.5, 1, 0))
final_alebeled_data$SD <- as.factor(ifelse(as.numeric(final_alebeled_data$SD) > 0.5, 1, 0))
final_alebeled_data$con <- as.factor(ifelse(as.numeric(final_alebeled_data$con) > 0.5, 1, 0))
summary(final_alebeled_data)
fp <- dplyr::filter(final_alebeled_data, false_positives == "1" & sov == "0" & EI == "0" & SD == "0" & con == "0")
final_data <- dplyr::anti_join(final_alebeled_data, fp, "AM2")
summary(final_data)
