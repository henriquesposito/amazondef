...
ama_model <- ama %>% 
  ...
  filter(location_cat != "Non Identified")  # remove the 3 obs
# get deforestation yearly (year before) - load data and merge
amazon_def_year <- readRDS("~/Documents/GitHub/amazondef/data/amazon_def_year.Rds")
amazon_def_year <- mutate(amazon_def_year, year = year + 1) %>% # yearly deforestation rates only come out the year after
  rename(def_year = `area km²`)
ama_model <- dplyr::left_join(ama_model, amazon_def_year, by = "year") # merge
...
# model
model <- multinom(mx_cat2 ~ location_cat + election_year + def_year + AAI, data = ama_model)
# get a nice table
stargazer::stargazer(model, type="text", add.lines = nrow(model$residuals), title = "Amazon Contructions by Location",
