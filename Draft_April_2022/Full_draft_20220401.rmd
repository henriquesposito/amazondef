...
pure_types_time_location <- ama_mx %>%
  count(president, mx_cat2, mx_cat, year, location_cat) %>%
  group_by(president, year) %>%
  mutate(perc = n / sum(n)) %>% 
  filter(mx_cat2 == "Pure-types") %>%
